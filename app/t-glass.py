...
def detect_absorption(j):
    ...
    if sell > thr and move < 0.02:
        j.set(SignalType.ABSORPTION, {"type": "BUYER", "sell_vol": sell},
              f"S={sell:,}", "BULLISH")
        logger.warning("🔥 ABS")
    elif buy > thr and move < 0.02:
        j.set(SignalType.ABSORPTION, {"type": "SELLER", "buy_vol": buy},
              f"B={buy:,}", "BEARISH")
        logger.warning("🔥 ABS")
...
def detect_price_response(j):
    ...
    if sell > thr and move >= -0.01:
        j.set(SignalType.PRICE_RESPONSE, {"direction": "BULLISH", "sell_vol": sell},
              f"S={sell:,}", "BULLISH")
        logger.warning("📊 PR")
    elif buy > thr and move <= 0.01:
        j.set(SignalType.PRICE_RESPONSE, {"direction": "BEARISH", "buy_vol": buy},
              f"B={buy:,}", "BEARISH")
        logger.warning("📊 PR")
...
def detect_imbalance(j):
    ...
    if ratio >= 2.0:
        j.set(SignalType.IMBALANCE, {"dominant": "BID", "ratio": ratio},
              f"R={ratio:.2f}", "BULLISH")
        logger.warning(f"⚖️ IMB BID")
    elif ratio <= 0.5:
        j.set(SignalType.IMBALANCE, {"dominant": "ASK", "ratio": ratio},
              f"R={ratio:.2f}", "BEARISH")
...
def detect_wall(j):
    ...
    for p, v in orderbook.curr_bids.items():
        if v >= sz:
            j.set(SignalType.WALL, {"side": "BID", "price": p, "size": v},
                  f"BID {p:.4f}", "BULLISH")
            logger.warning(f"🧱 WALL BID")
            return
    for p, v in orderbook.curr_asks.items():
        if v >= sz:
            j.set(SignalType.WALL, {"side": "ASK", "price": p, "size": v},
                  f"ASK {p:.4f}", "BEARISH")
...
def detect_liquidity_shift(j):
    if not hasattr(detect_liquidity_shift, "last_bid"):
        ...
        return
    bch = abs(orderbook.bid_size - detect_liquidity_shift.last_bid) / max(
        detect_liquidity_shift.last_bid, 1)
    ach = abs(orderbook.ask_size - detect_liquidity_shift.last_ask) / max(
        detect_liquidity_shift.last_ask, 1)
    detect_liquidity_shift.last_bid = orderbook.bid_size
    detect_liquidity_shift.last_ask = orderbook.ask_size
    if bch > 0.5:
...
def detect_volume_cluster(j):
    ...
    if len(large) >= 3:
        j.set(SignalType.VOLUME_CLUSTER, {"cluster_size": len(large)},
              f"{len(large)} prints", "")
...
def detect_trade_velocity(j):
    ...
    vel = len(recent) / 3
    if vel >= 15:
        j.set(SignalType.TRADE_VELOCITY, {"velocity": vel},
              f"{vel:.1f} t/s", "")
...
async def check_setup(price, volume, side):
    ...
    if score >= required:
        ...
        if res and res.direction != "NONE" and res.confidence >= getattr(config,
...
async def stats_loop():
    while True:
        ...
        logger.info(
            f"📊 STATS | {rt}s | T={stats.trades} | B={stats.book_updates} | "
            f"D={stats.delta:+,} | Q={stats.qualified} | A={stats.alerts} | "
            f"AI ok={stats.ai_ok} no={stats.ai_no} | DUP={stats.duplicates_skipped} | "
...
async def main():
    ...
    logger.warning("=" * 40)
    logger.warning(
        f"🚀 T-GLASS v19.1 (MAX) | {config.TICKER} | Score={getattr(config, 'SETUP_SCORE_REQUIRED', 100)}")
    logger.warning("=" * 40)
